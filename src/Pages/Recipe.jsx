import { AspectRatio, Box, Container,Heading, Flex, Image, List, Text } from "@chakra-ui/react";
...
const Recipe = () => {
  ...
  return (
   <Container boxShadow={'rgba(0,0,0.12) 0px 5px 10px'} border={'1px solid skyblue'} width={'70%'} borderRadius={'2xl'}>
     <Box m={5} p={6}>
      <Flex justifyContent={"space-evenly"} alignItems={"flex-start"}>
        ...
        <Flex flexDirection={"column"} p={5} justifyContent={"left"}>
          <Text color={'blue.500'} marginTop={6} fontWeight={'bold'}>{meal.strMeal}</Text>
          <Text color={'blue.500'}>{meal.strArea}</Text>
          <Heading color={'blue.500'} m={6}>Watch the video here....</Heading>
          <AspectRatio border={'none'} borderRadius={'lg'} p={6} m={2}>
            <iframe title="meal-video" src={meal.strYoutube.replace("watch?v=", "embed/")} allowFullScreen />
          </AspectRatio>
        </Flex>
    ...
    <Box p={5} m={5} bg={"blue.100"} borderRadius={'2xl'}>
      <Heading color={'blue.500'} textAlign={'center'} marginBottom={4} fontWeight={'bold'}>Key Ingredients</Heading>
    ...
    <Box p={5} m={2} bg={'blue.50'} borderRadius={'2xl'}>
      <Heading color={'blue.600'} fontWeight={'bold'} m={4} textAlign={'center'}>Cook like this</Heading>
